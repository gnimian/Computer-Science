...
class simonsays(Fl_Window):
	...
	def flashing(self, x):
		if self.buttons[x].color() != FL_WHITE:
			...
			if x == 0:
				sp.Popen(['mpv', '--really-quiet',"yellow.mp3"]) #sound effects. make sure to have these files in the same folder
			elif x == 1:
				sp.Popen(['mpv', '--really-quiet',"red.mp3"])
			elif x == 2:
				sp.Popen(['mpv', '--really-quiet',"blue.mp3"])
			elif x == 3:
				sp.Popen(['mpv', '--really-quiet',"green.mp3"])
			self.redraw()
